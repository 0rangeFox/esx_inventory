...
Get coords with 4 decimals

function getCoordsFromOwner(identifier)
    local x = tonumber(string.sub(string.match(identifier, 'x[-]?%d+[.%d+]'), 2))
    local y = tonumber(string.sub(string.match(identifier, 'y[-]?%d+[.%d+]'), 2))
    local z = tonumber(string.sub(string.match(identifier, 'z[-]?%d+[.%d+]'), 2))
    return vector3(x, y, z)
end
